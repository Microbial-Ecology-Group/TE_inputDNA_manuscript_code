...
group.ps.rare <- rarefy_even_depth(
  group.ps,
...
group.ps.rare.df <- as(sample_data(group.ps.rare), "data.frame")


sample_data(group.ps.rare)$SampleGroup <- as.factor(sample_data(group.ps.rare)$SampleGroup)

group.ps.rare.css <- phyloseq_transform_css(group.ps.rare, log = F)
group.ps.rare.css.df <- as(sample_data(group.ps.rare.css), "data.frame")
...
AllSamples.group.ps.rare.css.df <- as(sample_data(group.ps.rare.css),"data.frame")
AllSamples.group.ps.rare.css.dist <- vegdist(decostand(t(otu_table(group.ps.rare.css)), "hell"), "euclidean") 
AllSamples.group.ps.rare.css.ord <- vegan::metaMDS(comm = t(AllSamples.group.ps.rare.css.dist), distance = "none", try = 100, trymax = 999, autotransform = F)

AllSamples.plt_ord_by_Group_library <- plot_ordination(group.ps.rare.css, AllSamples.group.ps.rare.css.ord, color = "SampleGroup", shape = "Kit") +
  theme_bw() +
...
AllSamples.data.adonis <- adonis2(AllSamples.group.ps.rare.css.dist ~ SampleGroup, data = group.ps.rare.css.df, permutations = 9999)
AllSamples.data.adonis # 7e-04, R2 =0.13205 
