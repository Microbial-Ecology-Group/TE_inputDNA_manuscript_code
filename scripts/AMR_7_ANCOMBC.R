...
colnames(phyloseq::tax_table(DatasetA.data_AMR.ps))[4] <- "Species"

output_group_datasetA = ancombc2(data = DatasetA.data_AMR.ps, tax_level = "class",
...
pairwise_ancom_output_DatasetA <- output_group_datasetA$res
...
df_fig_Treatment_DatasetA_filtered <- df_fig_Treatment_DatasetA %>%
  ...
  dplyr::mutate(group = dplyr::case_when(
    group == "Kit_num3_rounded" ~ "From Days 0&1 to Days 5&6",
    group == "Kit_num5_Kit_num3_rounded" ~ "From Days 5&6 to Days 14&15",
    group == "Kit_num7_Kit_num5_rounded" ~ "From Days 14&15 to Days 20&21",
    TRUE ~ group)) %>%
  dplyr::filter(group %in% c("From Days 0&1 to Days 5&6", 
                             "From Days 5&6 to Days 14&15", 
                             "From Days 14&15 to Days 20&21")) %>%
  dplyr::mutate(group = factor(group, levels = c("From Days 0&1 to Days 5&6", 
                                                 "From Days 5&6 to Days 14&15", 
                                                 "From Days 14&15 to Days 20&21"))) %>%
  droplevels()
...
log_table_DatasetA <- ancom_output_DatasetA$bias_correct_log_table
...
# Make melted data
DatasetA.AMR_data_group.melt <- psmelt(DatasetA.AMR_data_group.ps)
...
BiasAdj_DatasetA.AMR_data_group.melt <- DatasetA.AMR_data_group.melt %>%
  left_join(scaled_counts_DatasetA.long, by = c("Species" = "taxon", "Sample" = "Sample"))

Species_avg_counts_DatasetA_by_Kit <- BiasAdj_DatasetA.AMR_data_group.melt %>%
  dplyr::group_by(Species, Kit_num) %>%
  dplyr::summarise(Avg_BiasAdj_Count = mean(BiasAdj_counts, na.rm = FALSE)) %>%
  ungroup() %>%
  dplyr::mutate(Kit_label = dplyr::case_when(
    Kit_num == 3 ~ "From Days 0&1 to Days 5&6",
    Kit_num == 5 ~ "From Days 5&6 to Days 14&15",
    Kit_num == 7 ~ "From Days 14&15 to Days 20&21",
    TRUE ~ as.character(Kit_num)  # Keep other Kit values unchanged
  ))


df_fig_Treatment_DatasetA_filtered_merged_data <- df_fig_Treatment_DatasetA_filtered %>%
  left_join(Species_avg_counts_DatasetA_by_Kit, by = c("group" = "Kit_label", "taxon" = "Species"))

# Extract taxonomic data from the phyloseq object
tax_data_DatasetA <- as.data.frame(tax_table(DatasetA.AMR_data_group.ps))

# Join the taxonomic data with your data frame
df_with_taxonomy_DatasetA <- df_fig_Treatment_DatasetA_filtered_merged_data %>%
  left_join(tax_data_DatasetA, by = c("taxon" = "Species"),relationship = "many-to-many")  # Assuming 'Species' is your key column to join
...
main_species_plot_DatasetA <- ggplot(df_with_taxonomy_DatasetA, aes(x = value, y = taxon, color = color)) +
  ...
  labs(x = "Log-Fold Change", y = "Taxon", title = "FECAL - logFC of AMR groups in NEG group over time") +
