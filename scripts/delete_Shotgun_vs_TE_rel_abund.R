...
sample_data(all_AMR_group.ps)$SampleGroup
...
all_AMR_group.ps.css <- phyloseq_transform_css(all_AMR_group.ps, log = F)
all_AMR_group.ps.css.df <- as(sample_data(all_AMR_group.ps.css), "data.frame")

# ord_explore(all_AMR_group.ps.css)
...
rel_abund <- transform_sample_counts(all_AMR_group.ps.css, function(x) {x/sum(x)}*100)
...
ps_class.css <- tax_glom(all_AMR_group.ps.css, taxrank = "class")
...
ps_AMR.dist <- vegdist(t(otu_table(all_AMR_group.ps.css)), method = "bray")
...
# Add metadata
ps_AMR.dendro.data$labels <- left_join(ps_AMR.dendro.data$labels, sample_data(all_AMR_group.ps.css.df), by = c("label" = "Sample"))
...
# Dendrogram plot
as.factor(gene_pos_table_ps_AMR$trt)
...
## ──────────────────────────────────────────────
## 1 •  palettes  (edit here if you change groups)
## ──────────────────────────────────────────────
sg_pal   <- c("Avian fecal"   = "#33a02c",
              "Bovine fecal"  = "#b15928",
              "Canine fecal"  = "#6a3d9a",
              "Human wastewater" = "#7cb9cb",
              "Porcine fecal" = "#FB9A99")

type_pal <- c("Shotgun" = "navyblue",
              "TE" = "gold2")     # expand if you have more levels


## make sure the factors have the intended levels / order
gene_pos_table_ps_AMR$trt  <- factor(gene_pos_table_ps_AMR$trt,
                                     levels = names(sg_pal))
gene_pos_table_ps_AMR$Type <- factor(gene_pos_table_ps_AMR$Type,
                                     levels = names(type_pal))

## ──────────────────────────────────────────────
## 2 •  dendrogram with two colour columns
##     • first geom_point  = SampleGroup  (same as before)
##     • second geom_point = Type         (slightly further right)
## ──────────────────────────────────────────────
## ----------------------------------------------------------------------
## dendrogram + two colour bars
## ----------------------------------------------------------------------
plt_dendr_class_ps_class <-
  ggplot(segment_data_ps_AMR) +
  
  ## dendrogram branches
  geom_segment(aes(x, y, xend = xend, yend = yend),
               linewidth = .75, lineend = "round", linejoin = "round") +
  
  ## 1️⃣  Sample-group column  ─ coloured body *and* border (shape 22)
  geom_point(
    data = gene_pos_table_ps_AMR,
    aes(x      = x,
        y      = y_center,
        fill   = trt,
        colour = trt),          # border = same colour → no black line
  ...
  ## 2️⃣  Sequencing-type column ─ solid square, no border (shape 15)
  geom_point(
    data = gene_pos_table_ps_AMR,
    aes(x      = x,
        y      = y_center,
        colour = Type),         # only *colour* is used
    shape   = 15,               # solid square, no border drawn
    size    = 8,
    stroke  = 0,                # just in case
...
scale_y_continuous(breaks = gene_pos_table_ps_AMR$y_center,
  ...
  ## scales / legends -------------------------------------------------
scale_fill_manual (name = "Sample group",    values = sg_pal) +
  scale_colour_manual(
    name   = "Sequencing type",
    values = c(type_pal, sg_pal),          # both palettes in one scale
    breaks = names(type_pal)) +            # show only ‘Shotgun’, ‘TE’
  guides(fill   = guide_legend(order = 1),
         colour = guide_legend(order = 2)) +
  
  ## theme ------------------------------------------------------------
labs(x = "Ward's distance", y = NULL) +
  theme_bw() +
...
png("figures/Fig_2_Shotgun_vs_TE_relative_abundance_bySampleGroup.png", width = 1800, height = 1200)
class_ps_class_plots
dev.off()
